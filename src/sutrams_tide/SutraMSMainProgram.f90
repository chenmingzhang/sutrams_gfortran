...
!                                                                       
      ...
      CHARACTER(len=80):: SIMULA(4) ! simulation type
      CHARACTER(len=80):: MSHTYP(2) ! mesh type
...
! FILE UNIT NOTATION:
! 1  -- TITAL_FLUID_MASS.DAT
! 2  -- NODEWISE_FLUID_MASS.DAT
! 3  -- TITAL_SOLUTE_MASS.DAT
! 4  -- NODEWISE_SOLUTE_MASS.DAT
! 5  -- TIDE.INP
! 6  -- SALT.INP
! 7  -- TIDALLY_AVERAGED_VELOCITY.DAT
! 8  -- SEEPAGE_FACE.DAT
! 9  -- NODE.DAT
! 10 -- ELEMENT.DAT

      OPEN(1,FILE='TOTAL_FLUID_MASS.DAT') ! CHENGJI 2013-09-03
      OPEN(2,FILE='NODEWISE_FLUID_MSSS.DAT')
      OPEN(3,FILE='TOTAL_SOLUTE_MASS.DAT')
      OPEN(4,FILE='NODEWISE_SOLUTE_MASS.DAT')
      OPEN(9,FILE='NODE.DAT')
      OPEN(10,FILE='ELEMENT.DAT')
      OPEN(20098,FILE='BOUNDARY_SALT_FLUX.DAT')
      OPEN(200988,FILE='BOUNDARY_WATER_FLUX.DAT')
      OPEN(2009888,FILE='BOUNDARY_SALINITY.DAT')
...
!                                                                       
!.....SIMULATION TIME                                                   
      CALL CPU_TIME(RT0) ! START TIME OF THE SIMULATION
...
!|*   E R R O R   O U T P U T   &   L O G                         * *|  
      cSMY = 'SUTRA.SMY'  ! SIMULATION SUMMARY FILE
      fSMY = 1 
...
!.....ASSIGN UNIT NUMBERS AND OPEN FILE UNITS FOR THIS SIMULATION       
      ...
      WRITE (*,*) 'Reading parameters from ET.INP...'
      CALL INPUTET ()
!                                                                       
!.....OUTPUT BANNER                                                     
      WRITE (fLST, 110)
...
!.....INPUT DATASET 1:  OUTPUT HEADING                                  
      ...
      READ (fINP,'(a80,/,a80)',iostat=ios) TITLE1, TITLE2 ! READ US FINE IF IOS=0, OTHERWISE CALL ERROR
...
!     STATEMENT                                                         
      NMULTI = 0 
      DO K = 2, 4 
      IF (INDEX(SIMULA(K), 'MULTI').GT.0) THEN ! if multi species, then nmulti=1
         NMULTI = 1 
         EXIT 
      ENDIF 
      ENDDO 
...
!.....TEST FOR ERRORS IN NUMBER OF SPECIFIED SPECIES                    
      IF (NSPE.LT.1) NSPE = 1  ! single species simulation
      IF (ME.NE.0) GOTO 2010  ! if single species simulation, me has to be either -1 or +1
...
!                                                                       
 2020 IF (KTYPE.LT.0) THEN 
      ...
      if (fSOB>0 .and. NOBS>0) &
        call ErrorIO('Error: Observation points cannot be specified in main &
		 input file if Specified Observation Locations are being used')
...
!******************* UNSSTR = UNSATURATED
!******************* SSFSTR = TRANSIENT FLOW
!******************* SSTSTR = TRANSIENT TRANSPORT
!******************* RDSTR = COLD
...
!.....TOTAL SIMULATION TIME
      CALL CPU_TIME(RT1) ! END TIME OF THE SIMULATION
      RTF=RT1-RT0 ! TOTAL SIMULATION TIME
!
