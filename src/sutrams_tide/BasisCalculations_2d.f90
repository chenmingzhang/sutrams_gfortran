...
!                                                                       
      ...
      PORG = 0.D0 
      THICKG = 0.0D0 
      DRWDTG = 0.0D0        !MT 24102017
	  CWG = 0.0D0        	!MT 26102017
      DO IL = 1, 4 
         II = (L - 1) * 4 + IL 
         I = IN (II) 
         ...
         DRWDTG = DRWDTG + DRWDT (I) * F (IL)  !MT 24102017 
!		 CWG = CWG + CWT (I) * F (IL)  		   !MT 26102017 
         DO K = 1, NSPE 
...
!.....RHOG = FUNCTION(UITER)                                            
!      DRWDU(NESP) = DRWDTG     !MT 24102017
      RHOG = RHOW0 
!      DO K = 1, NSPE 
!         RHOG = RHOG + DRWDU (K) * (UITERG (K) - URHOW0 (K) )       !MT - 12112017 - Commented out
!==================!MT - 12112017 - Calculated Density using eq. 8 by van Lopik et al. 2015
           TEMP = UITERG(NESP)
           SALIN = UITERG(2)
           RHOG = (999.9 + 2.034E-2*TEMP - 6.162E-3*(TEMP**2) + 2.261E-5*(TEMP**3) - 4.657E-8*(TEMP**4)) + (802*SALIN &
