...
!    logical :: LOk=.false.	!MT 04112017 - Commented out due to conflict during gfortran compilation
    integer (I4B) :: &
      NLSKIP
    integer (I4B) :: &
      i, &
...
01010&
    ...
    do n = 1, NodeZones
      allocate(ProdSorp(n)%prodf0(NSPE), &
               ...
               stat=ios)
      if(ios/=0) call ErrorIO('SutraZoneModule::RdZoneData could not allocate &
				 species variables for sorption and production parameter zones')
    end do
...
01030 continue
    if(nzn/=NodeZones) &
      call ErrorIO('Error in SutraZoneModule::RdZoneData  - ['//trim(Val2Char(NodeZones))//'] &
		node zones defined but only ['//trim(Val2Char(nzn))//'] zones found in zone file')
...
01050 continue
    if(nzn/=NSPE*NodeZones) &
      call ErrorIO('Error in SutraZoneModule::RdZoneData  - ['//trim(Val2Char(NSPE*NodeZones))//'] &
		node zones defined for ProdSorp but only ['//trim(Val2Char(nzn))//'] zones found in zone file')
...
01130&
    if(nze/=ElemZones) &
      call ErrorIO('Error in SutraZoneModule::RdZoneData  - ['//trim(Val2Char(ElemZones))//'] &
		element zones defined but only ['//trim(Val2Char(nze))//'] zones found in zone file')
