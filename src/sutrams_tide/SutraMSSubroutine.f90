...
!                                                                       
      ...
      USE CJCONTRL ! CHENGJI 2013-09-02
      USE M_TIDE
      USE M_CONTROL
...
!          CWT, &   !MT 26102017 - COMMENTED OUT
        ...
        INTEGER (I4B) :: KK ! CHENGJI 2013-09-02
        INTEGER (I4B) :: OUTSEEP ! CHENGJI 2015-08-03
        REAL(DP):: TDLEVEL,SAL
        REAL(DP):: TEMP,SALIN  !MT 24102017
		
        ALLOCATE (GNUPCJ(NBCN)) ! CHENGJI 2013-09-02
        DO 111 IP=1,NPBC
			GNUPCJ(IP)=GNUP
  111   CONTINUE
...
!.....SAVE VALUES FOR ENERGY TRANSPORT                                  
      CST = CS 
	  CWT = CW
!      CWT = CW		!MT 26102017 - COMMENTED OUT 
!                                                                       
...
! **********************************************************************
 1000 IT = IT + 1 
      ...
      IF (TLAB.EQ.1) THEN
        OPEN(5,FILE='TIDE.INP',STATUS='OLD')
        OPEN(6,FILE='SALT.INP',STATUS='OLD')
        READ(5,*) TDLEVEL
        IF(MOD(IT,ISTORE)==0) THEN
  		  REWIND(5)
        ENDIF  
        READ(6,*) SAL
      ELSE
...
!        ON THIS TIME STEP                                              
 ...
 1090 CONTINUE 
!=============================================================================!MT - 24102017
!=====SET DRWDT VALUE FOR THE CALCULATION BASED ON PREVIOUS CURRENT SALINITY AND TEMPERATURE
!===============================================SET CW DIFFERENT FOR SEAWATER AND FRESHWATER
!	  DO I = 1, NN 
!        DO K = 1, NSPE 
!           TEMP = UVEC(I,NESP)
!           SALINITY = UVEC(I,2)
!		    IF (SALINITY.LT.17.0E-3) THEN
!				DRWDT(I) = 3E-05*(TEMP**2) - 0.0075*TEMP + 0.0321
!				CWT(I) = 4182			!MT 26102017 - CW OF FRESHWATER - FROM INPUT FILE
!			ELSE
!				DRWDT(I) = 3E-05*(TEMP**2) - 0.0064*TEMP - 0.0558
!				CWT(I) = 4000			!MT 26102017 - CW OF FRESHWATER - FROM INPUT FILE
!			END IF
!        ENDDO
!	  ENDDO
!     WRITE(*,*) DRWDT, CWT
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
 1100 ITER = ITER + 1 
      if (fOTM>0) then
        WRITE (  *, 1102) IT, ITMAX, ITER, &      ! trim --> Returns the argument with trailing blanks removed
                          trim(adjustl(cTime(TSEC))), &  ! adjustl --> Adjusts a character string to the left, removing leading blanks and inserting trailing blanks 
...
!        RCITM1 (i) = RCIT (i) 
 2000 DO I = 1, NN
        PITER(I)  = PVEC(I)  ! PITER APPEARS FOR THE FIRST TIME 
        PVEL(I)   = PVEC(I)  ! PVEL APPEARS FOR THE FIRST TIME
        RCITM1(I) = RCIT(I)  ! RCITM1 APPEARS FOR THE FIRST TIME
...
!      endforall 
      DO I = 1, NN
        DO K = 1, NSPE
          UITER(I, K) = UVEC(I, K) ! UITER APPEARS FOR THE FIRST TIME
        END DO
      END DO
...
!==================!MT - 12112017 - Calculated Density using eq. 8 by van Lopik et al. 2015
           TEMP = UVEC(I,NESP)
           SALIN = UVEC(I,2)
           RCIT(I) = (999.9 + 2.034E-2*TEMP - 6.162E-3*(TEMP**2) + 2.261E-5*(TEMP**3) - 4.657E-8*(TEMP**4)) + (802*SALIN &
                      - 2.001*SALIN*TEMP + 1.677E-2*SALIN*(TEMP**2) - 3.06E-5*SALIN*(TEMP**3) - 1.613E-5*(SALIN**2)*(TEMP**2))
!==========================================================================================
        END DO
...
!      ENDFORALL
      DO IP = 1, NPBC
        QPLITR(IP) = GNUPCJ(IP) * ( SpecifiedPBC(IP)%P - PITER( IABS( SpecifiedPBC(IP)%node ) ) )  ! CHCENGJI 2013-09-02
      END DO
!                                                                       
...
!      ENDFORALL 
      DO IP = 1, NPBC
        QPLITR(IP) = GNUPCJ(IP) * ( SpecifiedPBC(IP)%P - PITER( IABS( SpecifiedPBC(IP)%node ) ) )  ! CHENGJI 2013-09-02
      END DO 
!                                                                       
...
!.........IF THE IMAXTARGET IS EXCEEDED
           ...
           SW_     = SW 
           QPLITR_ = QPLITR
           DRWDT_ = DRWDT     !MT 24102017
!	   CWT_ = CWT     !MT 24102017
        ENDIF
      END IF
!                                                                       
!                                                                       
...
!.....TEST IF P SOLUTION [(ML-1) = -1 OR 0]                             
         SELECT CASE (U_ML(K)-1)
          ...
          CASE DEFAULT
            MSErrorValue%cDataSet='SUT'
            call ErrorIO('SUTRA solution: Internal error testing if P solution (ML-1 must be -1, 0, 1) &
...
!        FOR THIS TIME STEP                                             
         IF (ITER.EQ.1.AND.IBCT.NE.4) &
           CALL BCTIME (IPBCT, IUBCT, IQSOPT, IQSOUT, GNUP, PM1, GNUPCJ, TDLEVEL, OUTSEEP) ! CHENGJI 2013-09-02                                                         
!                                                                       
...
!     FOR P AND/OR U                                                    
         CALL BC(U_ML(K), GNUPCJ) ! CHENGJI 2013-09-02
!                                                                       
         CALL CPU_TIME (RPTIME1) 
               ...
               selectcase (ML - 1) 
                !P AND U                                                
                 case ( - 1) 
                   ...
                   SW     = SW_ 
                   QPLITR = QPLITR_
                   DRWDT = DRWDT_       !MT 24102017
!				   CWT = CWT_       !MT 24102017
                !P ONLY                                                 
                 case (0) 
                   TSECP0 = TSEC 
                   DELTP  = DELTP_ 
...
!.......RESET VALUES FOR ENERGY TRANSPORT FOR BUDGET CALCULATIONS       
         CS = CST 
		 CW = CWT
!         CW = CWT 		!MT 26102017 - COMMENTED OUT
!                                                                       
...
!     SOLUTE MASS BUDGET                                                
     IF (KBUDG.EQ.1) &
           CALL BUDGET (ML, IBCT, VOL, SW, DSWDP, RHO,    &
                        ...
                        UM2, UIN, QUIN, IQSOU, UBC, IUBC, CS1, CS2, CS3, SL, SR, GNUPCJ) ! CHENGJI 2013-09-02 
      ENDIF 
!     
      CALL OUTNODE2 (PVEC, UVEC, SW, IN, X, Y, Z, TITLE1, TITLE2)
      
!      IF (AVGVEL.EQ.1) THEN
!         OPEN(7,FILE='TIDALLY_AVERAGED_VELOCITY.DAT')
!         CALL AVGVELOCITY(VMAG, VANG1, VANG2, IN, X, Y, Z, TITLE1, TITLE2)
!      ENDIF
