...
!                                                                       
      ...
      USE M_CONTROL ! Chengji_2015_11_19
...
!****************************** generating heteregeneous permeability field  
      real(dp):: ibound,ubound,len,t  ! Chengji_2014_09_27
      integer(dp):: nran
      real(dp),dimension(ne)::your_random
!                                                                       
      SAVE UTYPE, STYPE 

        open(0519,file='RANDOM_NUMBER.DAT') ! Chengji_2014_09_27
        open(0520,file='RANDOM_PERMEABILITY.DAT')
        ...
        call random_seed()  ! Chengji 2014-09-27
        do nran=1,ne
            call random_number(t)
            your_random(nran)=ibound+len*t
            write(0519,*) your_random(nran)
        end do
...
!.....SET SIGMAW, URHOW0, DRWDU, AND VISC0 TO BASE VALUES               
      ...
      DRWDU = BDRWDU 
      BVISC0 = BVISC0  
!===============!MT 21062016 - Revised codes for multispecies including energy transport
...
!.....Calculate summary of viscosity equation
      ...
      do k=1,NSPE
        if(k==NESP) then
          ...
          cViscosityEquation=trim(adjustl(cViscosityEquation))//' + (dViscosity/d'//trim(adjustl(SPNAME(K)))//' * &
...
!.....CHECK FOR CORRECT KEYWORDS                                        
  236   IF((trim(ADSMOD(K)) .EQ.'NONE') .OR.(trim(ADSMOD(K)) .EQ.'LINEAR') .OR. &
     &     (trim(ADSMOD(K)) .EQ.'FREUNDLICH') .OR.(trim(ADSMOD(K)) .EQ.'LANGMUIR') ) GOTO 238                                                     
        ...
        call ErrorIO('Error: unknown sorption type ['//trim(ADSMOD(K))//'specifed for species &
			'//trim(Val2Char(K))//' ['//trim(adjustl(SPNAME(K)))//']')
...
!           SET SPECIFIC PRESSURE STORATIVITY, SOP.                           
            if(fZON==0) NodeData(NodeMap(ii))%sop = (1.D0 - NodeData(NodeMap(ii))%por ) * NodeData(NodeMap(ii))%compma &
			 + NodeData(NodeMap(ii))%por * COMPFL
...
!.....INPUT DATASETS 15A AND 15B: ELEMENTWISE DATA                      
  ...
  509 FORMAT(/) 
         ...
         DO 550 LL = 1, NE 
            ...
            if(L>NE)  call ErrorIO('Error: Specified element number ['//trim(Val2Char(L))//'] exceeds NE &
				['//trim(Val2Char(NE))//']')
            backspace(fINP)
            if(fZON==0) then
            ...
            if(fZON==0) then
 !****************************** calculating either homogeneous or heteregeneous permeability values
            if(homo.eq.0) then
            ...
            elseif(homo.eq.1) then
               if(ll.le.73290) then           
                 ElemData(ElemMap(l))%pmax=ElemData(ElemMap(l))%pmax*PMAXFA*(1+amp*your_random(LL)) 
                 ElemData(ElemMap(l))%pmid=ElemData(ElemMap(l))%pmid*PMINFA*(1+amp*your_random(LL))
                 ElemData(ElemMap(l))%pmin=ElemData(ElemMap(l))%pmin*PMINFA*(1+amp*your_random(LL))
               else
                 ElemData(ElemMap(l))%pmax=ElemData(ElemMap(l))%pmax*PMAXFA 
                 ElemData(ElemMap(l))%pmid=ElemData(ElemMap(l))%pmid*PMIDFA 
                 ElemData(ElemMap(l))%pmin=ElemData(ElemMap(l))%pmin*PMINFA
              ...
              write(0520,'(3e15.7)') ElemData(ElemMap(l))%pmax,ElemData(ElemMap(l))%pmid, &
                                     ElemData(ElemMap(l))%pmin
!****************************************** end ****************************************************
...
!.....INPUT DATASETS 15A AND 15B: ELEMENTWISE DATA                      
 ...
 1509 FORMAT(/) 
         ...
         DO 1550 LL = 1, NE 
            ...
            if(fZON==0) then
 !*************************************** calculating permeability values
            if(homo.eq.0) then
              ElemData(ElemMap(l))%pmax = ElemData(ElemMap(l))%pmax * PMAXFA 
              ElemData(ElemMap(l))%pmin = ElemData(ElemMap(l))%pmin * PMINFA 
            elseif(homo.eq.1) then
             if(LL.le.73290) then 
              ...
              write(0520,'(3e15.7)') ElemData(ElemMap(l))%pmax,ElemData(ElemMap(l))%pmin
!***************************************** end ***************************************************
...
!.....TEST IF SLAMBDA IS TO BE READ
 ...
 1650    READ (fINP,*,iostat=ios) ILAMB 
         if(ios/=0) call ErrorIO('Error specifying element number for lambda modification. &
		 Confirm that CEQ is either "AVERAGE" or "GEOMETRIC"')
         if(ILAMB>NE)  call ErrorIO('Error: Specified element number ['//trim(Val2Char(ILAMB))//'] exceeds NE &
			 ['//trim(Val2Char(NE))//']')
