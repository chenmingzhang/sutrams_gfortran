...
!         RHO (I) = RHOW0 
!         DO 140 K = 1, NSPE 
!            RHO(I) = RHO(I) + DRWDU(K) * ( UITER(I, K) - URHOW0(K) )        !MT - 12112017 - Commented out
!==================!MT - 12112017 - Calculated Density using eq. 8 by van Lopik et al. 2015
           TEMP = UITER(I,NESP)
           SALIN = UITER(I,2)
           RHO(I) = (999.9 + 2.034E-2*TEMP - 6.162E-3*(TEMP**2) + 2.261E-5*(TEMP**3) - 4.657E-8*(TEMP**4)) + (802*SALIN &
                      - 2.001*SALIN*TEMP + 1.677E-2*SALIN*(TEMP**2) - 3.06E-5*SALIN*(TEMP**3) - 1.613E-5*(SALIN**2)*(TEMP**2))
!==========================================================================================
!            IF (I.GT.70548) THEN 
!                WRITE(*,*) I, K, UITER(I,K), DRWDU(K), RHO(I)
!            ENDIF
!  140    END DO
 !        IF (I.GT.70548) THEN 
 !              WRITE (*,*) I, RHO(I)
...
!        WRITE(*,*) I, CW
!        PAUSE
!============================================SET DRWDU & CW A FUNCTION OF SALINITY
!         DRWDU(NESP) = DRWDT(I)     !MT 24102017 
!         WRITE (*,*) CWT
!		 DO K = 1, NSPE
!			IF (K.EQ.NESP) THEN
