...
!.....INPUT INITIAL CONDITIONS FOR COLD START (UNIT-fICS DATA)            
  620 READ (fICS,*,iostat=ios) TSTART ! START TIME OF THE SIMULATION, NORMALLY IT IS 0
  ...
  625    END DO 
      ELSEIF (CPUNI.EQ.'NONUNIFORM') THEN 
         READ (fICS,*,iostat=ios) (PVEC (I), I = 1, NN) ! P VALUE IN THE ICS FILE IS PVEC
...
!     (SET QPLITR=0)          
!======================================================================!MT
!.....SET DRWDT VALUE BASED ON INITIAL SALINITY FROM ICS/RST FILE
!	  DO I = 1, NN 
!         DO K = 1, NSPE 
!           TEMP = UVEC(I,NESP)
!           SAL = UVEC(I,2)
...
!        DRWDU(NESP) = DRWDT(I)      !MT 24102017
        ...
        DO K = 1, NSPE 
           UM1 (I, K) = UVEC (I, K) 
           UM2 (I, K) = UVEC (I, K)  
           !RCIT (I) = RCIT (I) + DRWDU (K) * (UVEC (I, K) - URHOW0 (K))    !MT - 12112017 - Commented out
        ENDDO 
