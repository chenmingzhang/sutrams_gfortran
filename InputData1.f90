...
!.....Calculate summary of viscosity equation
      ...
      do k=1,NSPE
        if(k==NESP) then
          ...
          cViscosityEquation=trim(adjustl(cViscosityEquation))//' + (dViscosity/d'//trim(adjustl(SPNAME(K)))//' * '//trim(adjustl(SPNAME(K)))//')'
...
!.....CHECK FOR CORRECT KEYWORDS                                        
  236   IF((trim(ADSMOD(K)) .EQ.'NONE') .OR.(trim(ADSMOD(K)) .EQ.'LINEAR') .OR. &
     &     (trim(ADSMOD(K)) .EQ.'FREUNDLICH') .OR.(trim(ADSMOD(K)) .EQ.'LANGMUIR') ) GOTO 238                                                     
        ...
        call ErrorIO('Error: unknown sorption type ['//trim(ADSMOD(K))//'specifed for species '//trim(Val2Char(K))//' ['//trim(adjustl(SPNAME(K)))//']')
  ...
  244   FORMAT(11X,1PD15.4,5X,'FREUNDLICH DISTRIBUTION COEFFICIENT'       &
     &        /11X,1PD15.4,5X,'SECOND FREUNDLICH COEFFICIENT')               
        ...
        IF (trim(ADSMOD(K)) .EQ.'FREUNDLICH'.AND.CHI2 (K) .LE.0.D0) THEN 
          call ErrorIO('Error: second sorption coefficient for species '//trim(Val2Char(K))//' ['//trim(adjustl(SPNAME(K)))//'] must be greater than 0.0')
        ENDIF 
...
!           SET SPECIFIC PRESSURE STORATIVITY, SOP.                           
            if(fZON==0) NodeData(NodeMap(ii))%sop = (1.D0 - NodeData(NodeMap(ii))%por ) * NodeData(NodeMap(ii))%compma + NodeData(NodeMap(ii))%por * COMPFL
...
!.....INPUT DATASETS 15A AND 15B: ELEMENTWISE DATA                      
  ...
  509 FORMAT(/) 
         ...
         DO 550 LL = 1, NE 
            ...
            if(L>NE)  call ErrorIO('Error: Specified element number ['//trim(Val2Char(L))//'] exceeds NE ['//trim(Val2Char(NE))//']')
            backspace(fINP)
            if(fZON==0) then
...
!.......TEST IF L EXCEEDS THE NUMBER OF SPECIES                         
         ...
         if(ios/=0) call ErrorIO('Error specifying species number for dispersivity multiplier for species'//trim(Val2Char(IMULT))//' ['//trim(SPNAME(IMULT))//']')
         GOTO 1600 
...
!.....TEST IF SLAMBDA IS TO BE READ
 ...
 1650    READ (fINP,*,iostat=ios) ILAMB 
         if(ios/=0) call ErrorIO('Error specifying element number for lambda modification.  Confirm that CEQ is either "AVERAGE" or "GEOMETRIC"')
         if(ILAMB>NE)  call ErrorIO('Error: Specified element number ['//trim(Val2Char(ILAMB))//'] exceeds NE ['//trim(Val2Char(NE))//']')
!.......TEST FOR TERMINATION OF DATASET 15D                             
         ...
         if(ios/=0) call ErrorIO('Error specifying lambda modification for species'//trim(Val2Char(IMULT))//' ['//trim(SPNAME(IMULT))//']')
         lModifyLambdas = .TRUE.
